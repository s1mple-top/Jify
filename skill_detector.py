...
class SkillDetector:
    ...
    DETECTION_PROMPT = """你是一个工作流与方法论分析器。你的任务是从用户与 Jify 的协作历史中，检测「可复用的范式」——即用户反复执行、跨具体场景稳定成立的思考框架或操作流程，并将这些范式固化为可重复调用的 skill。

核心原则：**你沉淀的是「可复用范式」，而不是「某次具体任务的内容」**。判断标准永远是可复用性——把同一套范式套到不同目标上，核心逻辑是否依然成立；成立才值得沉淀为 skill。
